import{n as Q,F as S,E as j,j as e,i as I,o as V,p,r as b,u as k,q as U,v as c,L as P,w as B,x as L,y as Z}from"./index-1752f049.js";import{n as r}from"./emotion-styled.browser.esm-fcf23ecf.js";var T={exports:{}},Y="SECRET_DO_NOT_PASS_THIS_OR_YOU_WILL_BE_FIRED",q=Y,z=q;function M(){}function F(){}F.resetWarningCache=M;var E=function(){function A(n,s,d,m,a,x){if(x!==z){var u=new Error("Calling PropTypes validators directly is not supported by the `prop-types` package. Use PropTypes.checkPropTypes() to call them. Read more at http://fb.me/use-check-prop-types");throw u.name="Invariant Violation",u}}A.isRequired=A;function o(){return A}var i={array:A,bigint:A,bool:A,func:A,number:A,object:A,string:A,symbol:A,any:A,arrayOf:o,element:A,elementType:A,instanceOf:o,node:A,objectOf:o,oneOf:o,oneOfType:o,shape:o,exact:o,checkPropTypes:F,resetWarningCache:M};return i.PropTypes=i,i};T.exports=E();var N=T.exports;const l=Q(N),J="data:image/png;base64,",y="data:image/png;base64,",O="/project-01/assets/bottle-sign-in@2x-240239e6.png",R="/project-01/assets/bottle-for-sign-in@1x-611ab1e8.png",X="/project-01/assets/bottle-for-sign-in@2x-ad8ad7bb.png",K="/project-01/assets/bottle-for-sign-in@1x-899234c8.png",C="data:image/png;base64,",W="/project-01/assets/bottle-for-sign-in@2x-1e16e44b.png",D="/project-01/assets/background-element-main-page@2x-3b9a03f0.png",G=r.div`
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;

  @media (min-width: 768px) {
    flex-direction: row;
    justify-content: space-between;
  }
`,H=r(S)`
  display: flex;
  flex-direction: column;
  gap: 16px;
  margin-top: 24px;
  width: 320px;
  justify-content: center;

  @media (min-width: 768px) {
    width: 316px;
    margin-top: 40px;
  }
  @media (min-width: 1440px) {
    width: 364px;
    margin-top: 140px;
    margin-right: 104px;
    margin-left: auto;
  }
  h1 {
    font-size: 26px;
    font-weight: 500;
    line-height: 1.23;
    color: var(--primery-color-black);
  }
  label {
    position: relative;
  }
  input {
    padding: 12px 10px;
    width: 100%;
    font-size: 16px;
    line-height: 1.2;
    border-radius: 6px;
    border: 0;
    outline: 1px solid var(--secondary-color---5);
    color: var(--primery-color-blue);
  }
  input::placeholder {
    color: var(--secondary-color-lightblue);
  }
  input:focus {
    outline-color: var(--secondary-color---5);
  }
  input:invalid {
    color: var(--secondary-color-red);
    border: 1px solid var(--secondary-color-red);
  }
  input::-ms-reveal,
  input::-ms-clear {
    display: none;
  }
  a {
    width: fit-content;
    font-size: 16px;
    line-height: 1.25;
    color: var(--primery-color-black);
    transition: color ease-in-out 250ms;
  }
  a:hover {
    color: var(--secondary-color-orange);
  }
  svg {
    color: var(--primery-color-blue);
  }
`,$=r.button`
  padding: 8px 30px;
  font-size: 16px;
  line-height: 1.25;
  border-radius: 10px;
  border: none;
  background-color: var(--primery-color-blue);
  color: var(--primery-color-white);
  box-shadow: 0 4px 8px 0 rgba(64, 123, 255, 0.34);
  @media (min-width: 768px) {
    font-size: 18px;
  }
  :hover {
    box-shadow: 0 4px 14px 0 rgba(64, 123, 255, 0.54);
  }
`,v=r.span`
  display: block;
  margin-bottom: 8px;
  font-size: 18px;
  line-height: 1.33;
  color: var(--primery-color-black);
`,_=r.button`
  position: absolute;
  right: 10px;
  top: 44px;
  background: none;
  border: none;
  cursor: pointer;
  stroke: var(--primery-color-blue);
`,f=r(j)`
  margin-left: 4px;
  font-size: 14px;
  color: var(--secondary-color-orange);
`,t=({iconName:A,width:o,height:i,stroke:n,fill:s})=>e.jsx("svg",{width:o||"24px",height:i||"24px",children:e.jsx("use",{href:`${I}#${A}`,stroke:n,fill:s})});t.propTypes={iconName:l.string.isRequired,width:l.string,height:l.string,stroke:l.string,fill:l.string};const ee=V().shape({email:p().email("Please enter a valid email").required("E-mail is required"),password:p().min(8,"Password must be 8 or more characters").max(64).required("Password is required")}),Ae=()=>{const[A,o]=b.useState(!1),i=k(),n={email:"",password:""},s=({email:d,password:m},{resetForm:a})=>{event.preventDefault(),i(B({email:d,password:m})),a()};return e.jsx(U,{initialValues:n,validationSchema:ee,onSubmit:s,children:e.jsx(G,{children:e.jsxs(H,{children:[e.jsx("h1",{children:"Sign In"}),e.jsxs("label",{children:[e.jsx(v,{children:"Enter your email"}),e.jsx(c,{name:"email",type:"email",placeholder:"E-mail"}),e.jsx(f,{name:"email",component:"span"})]}),e.jsxs("label",{children:[e.jsx(v,{children:"Enter your password"}),e.jsxs("div",{children:[e.jsx(c,{name:"password",type:A?"text":"password",placeholder:"Password"}),e.jsx(_,{type:"button",onClick:()=>o(!A),children:A?e.jsx(t,{iconName:"icon-eye",width:"16px",height:"16px"}):e.jsx(t,{iconName:"icon-eye-slash",width:"16px",height:"16px"})})]}),e.jsx(f,{name:"password",component:"span"})]}),e.jsx($,{type:"submit",children:"Sign In"}),e.jsx(P,{to:"/signup",children:"Sign Up"})]})})})},re=r.div`
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;

  @media (min-width: 768px) {
    flex-direction: row;
    justify-content: space-between;
  }
`,oe=r(S)`
  display: flex;
  flex-direction: column;
  gap: 16px;
  margin-top: 24px;
  width: 320px;
  justify-content: center;

  @media (min-width: 768px) {
    width: 316px;
    margin-top: 40px;
  }
  @media (min-width: 1440px) {
    width: 364px;
    margin-top: 140px;
    margin-right: 104px;
    margin-left: auto;
  }
  h1 {
    font-size: 26px;
    font-weight: 500;
    line-height: 1.23;
    color: var(--primery-color-black);
  }
  label {
    position: relative;
  }
  input {
    padding: 12px 10px;
    width: 100%;
    font-size: 16px;
    line-height: 1.2;
    border-radius: 6px;
    border: 0;
    outline: 1px solid var(--secondary-color---5);
    color: var(--primery-color-blue);
  }
  input::placeholder {
    color: var(--secondary-color-lightblue);
  }
  input:focus {
    outline-color: var(--secondary-color---5);
  }
  input:invalid {
    color: var(--secondary-color-red);
    border: 1px solid var(--secondary-color-red);
  }
  input::-ms-reveal,
  input::-ms-clear {
    display: none;
  }
  a {
    width: fit-content;
    font-size: 16px;
    line-height: 1.25;
    color: var(--primery-color-black);
    transition: color ease-in-out 250ms;
  }
  a:hover {
    color: var(--secondary-color-orange);
  }
  svg {
    color: var(--primery-color-blue);
  }
`,ie=r.button`
  padding: 8px 30px;
  font-size: 16px;
  line-height: 1.25;
  border-radius: 10px;
  border: none;
  background-color: var(--primery-color-blue);
  color: var(--primery-color-white);
  box-shadow: 0 4px 8px 0 rgba(64, 123, 255, 0.34);
  @media (min-width: 768px) {
    font-size: 18px;
  }
  :hover {
    box-shadow: 0 4px 14px 0 rgba(64, 123, 255, 0.54);
  }
`,h=r.span`
  display: block;
  margin-bottom: 8px;
  font-size: 18px;
  line-height: 1.33;
  color: var(--primery-color-black);
`,w=r.button`
  position: absolute;
  right: 10px;
  top: 44px;
  background: none;
  border: none;
  cursor: pointer;
  stroke: var(--primery-color-blue);
`,g=r(j)`
  margin-left: 4px;
  font-size: 14px;
  color: var(--secondary-color-orange);
`,te=V().shape({email:p().email("Please enter a valid email").required("E-mail is required"),password:p().min(8,"Password must be 8 or more characters").max(64).required("Password is required"),repeatPassword:p().oneOf([L("password"),null],"The passwords do not match").required("Repeat password field is required")}),ne=()=>{const[A,o]=b.useState(!1),[i,n]=b.useState(!1),s=k(),d={email:"",password:"",repeatPassword:""},m=({email:a,password:x},{resetForm:u})=>{event.preventDefault(),s(Z({email:a,password:x})),u()};return e.jsx(U,{initialValues:d,validationSchema:te,onSubmit:m,children:a=>e.jsx(re,{children:e.jsxs(oe,{children:[e.jsx("h1",{children:"Sign Up"}),e.jsxs("label",{children:[e.jsx(h,{children:"Enter your email"}),e.jsx(c,{name:"email",type:"email",placeholder:"E-mail"}),e.jsx(g,{name:"email",component:"span"})]}),e.jsxs("label",{children:[e.jsx(h,{children:"Enter your password"}),e.jsxs("div",{children:[e.jsx(c,{name:"password",type:A?"text":"password",placeholder:"Password"}),e.jsx(w,{type:"button",onClick:()=>o(!A),children:A?e.jsx(t,{iconName:"icon-eye",width:"16px",height:"16px"}):e.jsx(t,{iconName:"icon-eye-slash",width:"16px",height:"16px"})})]}),e.jsx(g,{name:"password",component:"span"})]}),e.jsxs("label",{children:[e.jsx(h,{children:"Repeat password"}),e.jsxs("div",{children:[e.jsx(c,{name:"repeatPassword",type:i?"text":"password",placeholder:"Password"}),e.jsx(w,{type:"button",onClick:()=>n(!i),children:i?e.jsx(t,{iconName:"icon-eye",width:"16px",height:"16px"}):e.jsx(t,{iconName:"icon-eye-slash",width:"16px",height:"16px"})})]}),e.jsx(g,{name:"repeatPassword",component:"span"})]}),e.jsx(ie,{type:"submit",children:"Sign Up"}),e.jsx(P,{to:"/signin",onClick:()=>{a.handleReset()},children:"Sign in"})]})})})},pe=({isRegister:A=!1})=>A?e.jsx(ne,{}):e.jsx(Ae,{}),ce=r.div`
  margin: 0 auto;
  width: 100%;
  padding-left: 32px;
  padding-right: 32px;
`,de=r.div`
  width: 100%;
  height: 100vh;
  position: absolute;
  top: 60px;
  left: 0;
  z-index: -10;
  background-image: url(${J}), url(${y});
  background-size: 100%, cover;
  background-repeat: no-repeat;
  background-position:
    top 400px left 20px,
    center;

  @media (min-device-pixel-ratio: 2),
    (-webkit-min-device-pixel-ratio: 2),
    (min-resolution: 192dpi),
    (min-resolution: 2dppx) {
    background-image: url(${O}), url(${y});
  }

  @media (min-width: 768px) {
    margin: 0 auto;
    max-width: 768px;
    width: 100%;
    height: 100vh;
    top: -20px;
    left: 0;
    right: 30px;
    background-image: url(${R});
    background-size: cover;
    background-position: center;

    @media (min-device-pixel-ratio: 2),
      (-webkit-min-device-pixel-ratio: 2),
      (min-resolution: 192dpi),
      (min-resolution: 2dppx) {
      background-image: url(${X});
    }
  }

  @media (min-width: 1440px) {
    margin: 0 auto;
    max-width: 1404px;
    width: 100%;
    height: 100vh;
    top: 60px;
    left: 18px;
    background-image: url(${K}), url(${C});
    background-size: 916px, 1404px;
    background-position:
      top 28px left -51px,
      center;

    @media (min-device-pixel-ratio: 2),
      (-webkit-min-device-pixel-ratio: 2),
      (min-resolution: 192dpi),
      (min-resolution: 2dppx) {
      background-image: url(${W}), url(${D});
    }
  }
`,me=r.p`
  margin-top: 20px;
  width: 280px;
  font-size: 18px;
  text-align: center;
  color: var(--secondary-color-red);
  @media (min-width: 768px) {
    width: 320px;
    margin-right: auto;
    font-size: 20px;
  }
  @media (min-width: 1440px) {
    margin-left: auto;
    margin-right: 133px;
  }
`;export{pe as A,ce as S,y as a,O as b,X as c,K as d,C as e,W as f,D as g,de as h,me as i,J as m,R as t};
