import{F as y,E as S,l as f,m as i,r as m,e as w,j as A,n as V,o as t,L as M,p as Q,q as F,v as k}from"./index-f20119f8.js";import{n as r,G as U}from"./emotion-styled.browser.esm-f17daeb1.js";const O="data:image/png;base64,",C="data:image/png;base64,",N="/project-01/assets/bottle-sign-in@2x-240239e6.png",R="/project-01/assets/bottle-for-sign-in@1x-611ab1e8.png",W="/project-01/assets/bottle-for-sign-in@2x-ad8ad7bb.png",G="/project-01/assets/bottle-for-sign-in@1x-899234c8.png",D="data:image/png;base64,",H="/project-01/assets/bottle-for-sign-in@2x-1e16e44b.png",$="/project-01/assets/background-element-main-page@2x-3b9a03f0.png",B=r(y)`
  display: flex;
  flex-direction: column;
  gap: 16px;
  margin-top: 24px;
  width: 100%;
  @media (min-width: 768px) {
    width: 336px;
    margin-top: 40px;
  }
  @media (min-width: 1440px) {
    width: 384px;
    margin-top: 140px;
    margin-right: 104px;
    margin-left: auto;
  }
  h1 {
    font-size: 26px;
    font-weight: 500;
    line-height: 1.23;
    color: var(--primery-color-black);
  }
  label {
    position: relative;
  }
  input {
    padding: 12px 10px;
    width: 100%;
    font-size: 16px;
    line-height: 1.2;
    border-radius: 6px;
    border: 0;
    outline: 1px solid var(--secondary-color---5);
    color: var(--primery-color-blue);
  }
  input::placeholder {
    color: var(--secondary-color-lightblue);
  }
  input:focus {
    outline-color: var(--secondary-color---5);
  }
  input:invalid {
    color: var(--secondary-color-red);
    border: 1px solid var(--secondary-color-red);
  }
  input::-ms-reveal,
  input::-ms-clear {
    display: none;
  }
  a {
    width: fit-content;
    font-size: 16px;
    line-height: 1.25;
    color: var(--primery-color-black);
    transition: color ease-in-out 250ms;
  }
  a:hover {
    color: var(--secondary-color-orange);
  }
  svg {
    color: var(--primery-color-blue);
  }
`,P=r.button`
  padding: 8px 30px;
  font-size: 16px;
  line-height: 1.25;
  border-radius: 10px;
  border: none;
  background-color: var(--primery-color-blue);
  color: var(--primery-color-white);
  box-shadow: 0 4px 8px 0 rgba(64, 123, 255, 0.34);
  @media (min-width: 768px) {
    font-size: 18px;
  }
  :hover {
    box-shadow: 0 4px 14px 0 rgba(64, 123, 255, 0.54);
  }
`,g=r.span`
  display: block;
  margin-bottom: 8px;
  font-size: 18px;
  line-height: 1.33;
  color: var(--primery-color-black);
`,Z=r.button`
  position: absolute;
  right: 10px;
  top: 44px;
  background: none;
  border: none;
  cursor: pointer;
  stroke: var(--primery-color-blue);
`,v=r(S)`
  margin-left: 4px;
  font-size: 14px;
  color: var(--secondary-color-orange);
`;function x(e){return U({tag:"svg",attr:{fill:"none",viewBox:"0 0 24 24",strokeWidth:"1.5",stroke:"currentColor","aria-hidden":"true",dataSlot:"icon"},child:[{tag:"path",attr:{strokeLinecap:"round",strokeLinejoin:"round",d:"M3.98 8.223A10.477 10.477 0 0 0 1.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.451 10.451 0 0 1 12 4.5c4.756 0 8.773 3.162 10.065 7.498a10.522 10.522 0 0 1-4.293 5.774M6.228 6.228 3 3m3.228 3.228 3.65 3.65m7.894 7.894L21 21m-3.228-3.228-3.65-3.65m0 0a3 3 0 1 0-4.243-4.243m4.242 4.242L9.88 9.88"},child:[]}]})(e)}function h(e){return U({tag:"svg",attr:{fill:"none",viewBox:"0 0 24 24",strokeWidth:"1.5",stroke:"currentColor","aria-hidden":"true",dataSlot:"icon"},child:[{tag:"path",attr:{strokeLinecap:"round",strokeLinejoin:"round",d:"M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z"},child:[]},{tag:"path",attr:{strokeLinecap:"round",strokeLinejoin:"round",d:"M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"},child:[]}]})(e)}const I=f().shape({email:i().email("Please enter a valid email").required("E-mail is required"),password:i().min(8,"Password must be 8 or more characters").max(64).required("Password is required")}),Y=()=>{const[e,a]=m.useState(!1),o=w(),n={email:"",password:""},l=({email:c,password:p},{resetForm:s})=>{event.preventDefault(),o(Q({email:c,password:p})),s()};return A.jsx(V,{initialValues:n,validationSchema:I,onSubmit:l,children:A.jsxs(B,{children:[A.jsx("h1",{children:"Sign In"}),A.jsxs("label",{children:[A.jsx(g,{children:"Enter your email"}),A.jsx(t,{name:"email",type:"email",placeholder:"E-mail"}),A.jsx(v,{name:"email",component:"span"})]}),A.jsxs("label",{children:[A.jsx(g,{children:"Enter your password"}),A.jsxs("div",{children:[A.jsx(t,{name:"password",type:e?"text":"password",placeholder:"Password"}),A.jsx(Z,{type:"button",onClick:()=>a(!e),children:e?A.jsx(h,{}):A.jsx(x,{})})]}),A.jsx(v,{name:"password",component:"span"})]}),A.jsx(P,{type:"submit",children:"Sign In"}),A.jsx(M,{to:"/signup",children:"Sign Up"})]})})},q=r(y)`
  display: flex;
  flex-direction: column;
  gap: 16px;
  margin-top: 24px;
  width: 100%;
  @media (min-width: 768px) {
    width: 336px;
    margin-top: 40px;
  }
  @media (min-width: 1440px) {
    width: 384px;
    margin-top: 140px;
    margin-right: 104px;
    margin-left: auto;
  }
  h1 {
    font-size: 26px;
    font-weight: 500;
    line-height: 1.23;
    color: var(--primery-color-black);
  }
  label {
    position: relative;
  }
  input {
    padding: 12px 10px;
    width: 100%;
    font-size: 16px;
    line-height: 1.2;
    border-radius: 6px;
    border: 0;
    outline: 1px solid var(--secondary-color---5);
    color: var(--primery-color-blue);
  }
  input::placeholder {
    color: var(--secondary-color-lightblue);
  }
  input:focus {
    outline-color: var(--secondary-color---5);
  }
  input:invalid {
    color: var(--secondary-color-red);
    border: 1px solid var(--secondary-color-red);
  }
  input::-ms-reveal,
  input::-ms-clear {
    display: none;
  }
  a {
    width: fit-content;
    font-size: 16px;
    line-height: 1.25;
    color: var(--primery-color-black);
    transition: color ease-in-out 250ms;
  }
  a:hover {
    color: var(--secondary-color-orange);
  }
  svg {
    color: var(--primery-color-blue);
  }
`,T=r.button`
  padding: 8px 30px;
  font-size: 16px;
  line-height: 1.25;
  border-radius: 10px;
  border: none;
  background-color: var(--primery-color-blue);
  color: var(--primery-color-white);
  box-shadow: 0 4px 8px 0 rgba(64, 123, 255, 0.34);
  @media (min-width: 768px) {
    font-size: 18px;
  }
  :hover {
    box-shadow: 0 4px 14px 0 rgba(64, 123, 255, 0.54);
  }
`,d=r.span`
  display: block;
  margin-bottom: 8px;
  font-size: 18px;
  line-height: 1.33;
  color: var(--primery-color-black);
`,j=r.button`
  position: absolute;
  right: 10px;
  top: 44px;
  background: none;
  border: none;
  cursor: pointer;
  stroke: var(--primery-color-blue);
`,u=r(S)`
  margin-left: 4px;
  font-size: 14px;
  color: var(--secondary-color-orange);
`,z=f().shape({email:i().email("Please enter a valid email").required("E-mail is required"),password:i().min(8,"Password must be 8 or more characters").max(64).required("Password is required"),repeatPassword:i().oneOf([F("password"),null],"The passwords do not match").required("Repeat password field is required")}),J=()=>{const[e,a]=m.useState(!1),[o,n]=m.useState(!1),l=w(),c={email:"",password:"",repeatPassword:""},p=({email:s,password:b},{resetForm:L})=>{event.preventDefault(),l(k({email:s,password:b})),L()};return A.jsx(V,{initialValues:c,validationSchema:z,onSubmit:p,children:s=>A.jsxs(q,{children:[A.jsx("h1",{children:"Sign Up"}),A.jsxs("label",{children:[A.jsx(d,{children:"Enter your email"}),A.jsx(t,{name:"email",type:"email",placeholder:"E-mail"}),A.jsx(u,{name:"email",component:"span"})]}),A.jsxs("label",{children:[A.jsx(d,{children:"Enter your password"}),A.jsxs("div",{children:[A.jsx(t,{name:"password",type:e?"text":"password",placeholder:"Password"}),A.jsx(j,{type:"button",onClick:()=>a(!e),children:e?A.jsx(h,{}):A.jsx(x,{})})]}),A.jsx(u,{name:"password",component:"span"})]}),A.jsxs("label",{children:[A.jsx(d,{children:"Repeat password"}),A.jsxs("div",{children:[A.jsx(t,{name:"repeatPassword",type:o?"text":"password",placeholder:"Password"}),A.jsx(j,{type:"button",onClick:()=>n(!o),children:o?A.jsx(h,{}):A.jsx(x,{})})]}),A.jsx(u,{name:"repeatPassword",component:"span"})]}),A.jsx(T,{type:"submit",children:"Sign Up"}),A.jsx(M,{to:"/signin",onClick:()=>{s.handleReset()},children:"Sign in"})]})})},_=({isRegister:e=!1})=>e?A.jsx(J,{}):A.jsx(Y,{});export{_ as A,C as a,N as b,W as c,G as d,D as e,H as f,$ as g,O as m,R as t};
