...
export default function EditWaterModal({ onClose, modalData }) {
  ...
  const [time, setTime] = useState(modalData ? modalData.time : '00:00');
  const [amount, setAmount] = useState(modalData ? modalData.waterVolume : 250);
  const [waterValue, setWaterValue] = useState(
    modalData ? modalData.waterVolume : 250
  );
    ...
    <>
      ...
      <div className={css.edit_water_modal}>
        ...
        <div className={css.water_value_button_send}>
          ...
          <button className={css.button_save} type="submit" onClick={onClose}>
            Save
          </button>
        </div>
      </div>
